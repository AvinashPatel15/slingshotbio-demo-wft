...
const heroStats = [
  ...
  { value: "<10 CV", label: "Lot-To-Lot" },
  { value: "Non-Biohazardous", label: "Materials" }
];
      ...
      <div className="frame h-full relative">
        ...
        >
          {heroStats.map((stat, index) => (
            <div
              key={stat.value}
              className={`flex flex-row justify-between items-center w-full px-6 md:px-8 ${
                index < heroStats.length - 1 ? "border-b-[0.8px] border-[#3D3D3D]" : ""
              }`}
              style={{
                height: "84.8px",
                boxSizing: "border-box",
            ...
            >
              <span className="font-heading font-bold text-[24px] leading-[28px] tracking-[-1.28px] text-[#F76A00] flex items-center">
                {stat.value}
              </span>
              <span className="font-heading font-bold text-[16px] leading-[20px] text-white flex items-center capitalize">
                {stat.label}
              </span>
            </div>
          ))}
